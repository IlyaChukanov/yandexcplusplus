...
public:
  using TreeHolder = std::weak_ptr<SummingSegmentTree<Data, BulkOperation>>;
...
public:
  BulkLinearUpdater(const TreeHolder& tree = TreeHolder()) : BaseBulkOperation(tree) {}
...
private:
  struct Node;
  using NodeHolder = std::shared_ptr<Node>;
  struct Node {
    NodeHolder left;
    NodeHolder right;
  ...
  static NodeHolder Build(IndexSegment segment) {
    if (segment.empty()) {
      ...
      return std::make_shared<Node>(Node{nullptr, nullptr, segment});
    } else {
      const size_t middle = segment.left + segment.length() / 2;
      return std::make_shared<Node>(Node{Build({segment.left, middle}), Build({middle, segment.right}), segment});
    }
  }

  template <typename Visitor>
  static typename Visitor::ResultType TraverseWithQuery(NodeHolder node, IndexSegment query_segment, Visitor visitor) {
  ...
  public:
    ...
    Data ProcessEmpty(NodeHolder) const {
      return {};
    }

    Data ProcessFull(NodeHolder node) const {
      return node->data;
    }

    Data ProcessPartial(NodeHolder, IndexSegment, const Data& left_result, const Data& right_result) const {
  ...
  public:
    ...
    void ProcessEmpty(NodeHolder) const {}

    void ProcessFull(NodeHolder node) const {
      node->postponed_bulk_operation.CombineWith(operation_);
      node->data = operation_.Collapse(node->data, node->segment);
    }

    void ProcessPartial(NodeHolder node, IndexSegment) const {
  ...
  static void PropagateBulkOperation(NodeHolder node) {
    for (const auto& child_ptr : {node->left, node->right}) {
      if (child_ptr) {
...
public:
  ...
  BudgetManager() : tree_(std::make_shared<SummingSegmentTree<MoneyState, BulkLinearUpdater>>(DAY_COUNT)) {}
...
RequestHolder Request::Create(Request::Type type) {
  switch (type) {
    case Request::Type::COMPUTE_INCOME: {
      return std::make_unique<ComputeIncomeRequest>();
    }
    case Request::Type::EARN: {
      return std::make_unique<AddMoneyRequest<+1>>();
    }
    case Request::Type::SPEND: {
      return std::make_unique<AddMoneyRequest<-1>>();
    }
    case Request::Type::PAY_TAX: {
      return std::make_unique<PayTaxRequest>();
    }
    default:
      return nullptr;
  }
...
vector<double> ProcessRequests(const vector<RequestHolder>& requests) {
  ...
  for (const auto& request_holder : requests) {
    if (request_holder->type == Request::Type::COMPUTE_INCOME) {
      const auto& request = dynamic_cast<const ComputeIncomeRequest&>(*request_holder);
