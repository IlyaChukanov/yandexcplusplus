...
LayersType LayersMapping(const std::string& str) {
  if (str == "bus_lines") return LayersType::BUS;
  if (str == "bus_labels") return LayersType::BUS_NAME;
  if (str == "stop_points") return LayersType::STOP;
  if (str == "stop_labels") return LayersType::STOP_NAME;
...
RenderParams DatabaseManager::ExtractRenderParams(const Json::Node &node) {
  ...
  {
    auto layers = param_map.at("layers").AsArray();
    for (const auto& str : layers) {
      rp.layers_order.push_back(LayersMapping(str.AsString()));
    }
  }
  ...
  {
    auto point = param_map.at("bus_label_offset").AsArray();
    rp.bus_label_offset = {.x = point[0].AsDouble(),
                           .y = point[1].AsDouble()};
  }
