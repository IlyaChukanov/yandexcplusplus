...
public:
  ...
  WriteAccess operator[](const K& key) {
    size_t bucket_key = GetBucketIdx(key);
    auto& curr_bucket = buckets_[bucket_key];
    return WriteAccess{std::lock_guard(curr_bucket.mutex_), curr_bucket.map_[key]};
  }

  ReadAccess At(const K& key) const {
    size_t bucket_key = GetBucketIdx(key);
    auto& curr_bucket = buckets_[bucket_key];
    return ReadAccess{std::lock_guard(curr_bucket.mutex_), curr_bucket.map_.at(key)};
  }

  bool Has(const K& key) const {
    return buckets_[GetBucketIdx(key)].map_.count(key);
  }
