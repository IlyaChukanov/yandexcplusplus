...
TEST(TestModifyRequest, TestAddStopRequest3) {
  auto req = std::make_unique<AddStopRequest>();
  ASSERT_EQ(req->GetType(), Request::Type::ADD_STOP);
  std::string request{"S1: 22.22, 33.33, 1m to S2, 2m to S3, 3m to S4"};
  req->ParseFrom(request);
  Database db;
  req->Process(db);
  ...
  ASSERT_DOUBLE_EQ(stop->distance_to_stop.at("S4"), 3);
}
...
TEST(TestReadRequest, TestTakeRoute1) {
  ...
  std::string answer_str = "Bus Bus1: 4 stops on route, 3 unique stops, 0 route length, -nan curvature";
