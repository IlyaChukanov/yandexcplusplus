...
struct Hasher {
  size_t operator()(const Point3D& point) const {
    const size_t x = 363277;
    size_t x_hash = coord_hash(point.x);
    size_t y_hash = coord_hash(point.y);
    size_t z_hash = coord_hash(point.z);
    return x_hash*x_hash*x + y_hash*x + z_hash;
  }
  std::hash<CoordType> coord_hash;
};
