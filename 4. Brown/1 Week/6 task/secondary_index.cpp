...
public:
  bool Put(const Record& record) {
    if (!database_.count(record.id)) {
      ...
      timestamp_range[record.timestamp].insert(record.id);
      karma_range[record.karma].insert(record.id);
  ...
  bool Erase(const string& id) {
    if (database_.count(id)) {
      timestamp_range[database_[id].timestamp].erase(id);
      karma_range[database_[id].karma].erase(id);
  ...
  void RangeByTimestamp(int low, int high, Callback callback) const {
    ...
    bool flag = true;
    for (auto i = range_begin; i != range_end && flag; ++i) {
      for (auto j = i->second.begin(); j != i->second.end(); ++j) {
        flag = callback(database_.at(*j));
      }
    }
  }
 ...
 private:
  ...
  // Timestamp - ID
  std::map<int, std::unordered_set<std::string>> timestamp_range;
  // Karma - ID
  std::map<int, std::unordered_set<std::string>> karma_range;
  // User - ID
