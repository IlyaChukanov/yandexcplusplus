...
struct AddressHasher {
  size_t operator()(const Address& adr) const {
    size_t city_hash = shash(adr.city);
    size_t street_hash = shash(adr.street);
    size_t building_hash = ihash(adr.building);
    return street_hash * street_hash * prime_root +
           city_hash * prime_root +
           building_hash;
  }
  const size_t prime_root = 2125001;
...
struct PersonHasher {
  size_t operator()(const Person& per) const {
    size_t name_hash = shash(per.name);
    size_t height_hash = ihash(per.height);
    size_t weight_hash = dhash(per.weight);
    size_t address_hash = ahash(per.address);
    return name_hash * name_hash * name_hash * x +
           address_hash * address_hash * x +
           weight_hash * x +
           height_hash;
  }
  const size_t x = 2155003;
  std::hash<std::string> shash;
  std::hash<int> ihash;
