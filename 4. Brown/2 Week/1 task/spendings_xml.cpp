...
vector<Spending> LoadFromXml(istream& input) {
  ...
  const auto childrens = root.Children();
  std::vector<Spending> result;
  for (const auto& child : childrens) {
    Spending spended;
    spended.category = child.AttributeValue<std::string>("category");
    spended.amount = child.AttributeValue<int>("amount");
    result.push_back(std::move(spended));
  }
  return result;
}
